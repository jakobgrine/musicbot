...
class CustomHelpCommand(commands.DefaultHelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        embed = discord.Embed(title=':scroll: Help', color=discord.Color.gold()) \
            .set_footer(text=f'Type\u2002{self.clean_prefix}help [command]\u2002for more info on a command. You can also type\u2002{self.clean_prefix}help [category]\u2002for more info on a category.')
        ...
        for category, commands in to_iterate:
            ...
            embed.add_field(name=category, value=', '.join([f'`{self.clean_prefix}{command.name}`' for command in commands]), inline=False)
    ...
    async def send_cog_help(self, cog):
        ...
        if commands:
            embed.add_field(name='Commands', value=', '.join([f'`{self.clean_prefix}{command.name}`' for command in commands]), inline=False)
        else:
